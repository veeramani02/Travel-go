...
function Payments() {
  ...
  // ─── Step 1: Create a DB payment record and update it after success ──────────
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const options = {
        ...
        amount: orderData.order.amount,             // paise from backend
        currency: orderData.order.currency,
        name: "Trip Booking",
        description: `${trip.pickupCity} → ${trip.destinationCity}`,
        order_id: orderData.order.id,
        method: paymentMethod === "upi" ? { upi: true } : undefined, // pre-select UPI tab
        prefill: {
          // fill from your auth context / profile if available
        },
        theme: { color: "#2563eb" },
        
        // 4. On payment success → verify signature → record in DB
  ...
  // ─── Route the correct handler based on payment method ───────────────────────
        ...
        <div className="card-body">
          ...
          {step === 2 && (
            <>
              ...
              {/* Card & UPI: button triggers Razorpay checkout */}
              {paymentMethod === "card" && (
                <CardPayment
              ...
              {/* Cash & Dues: existing server-side flow */}
              {paymentMethod === "cash" && (
                <CashPayment
                  handlePayment={handlePaymentByMethod}
                  loading={loading}
